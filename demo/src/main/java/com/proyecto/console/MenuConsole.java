...
import com.proyecto.dao.Usuario;
import com.proyecto.enums.EnumRolType;
...
public class MenuConsole {
    ...
    public void inputMenuAutor()
    {
        ...
        try
        {
            ...
            String codUnicoAutor = new OperacionesGenerales().guardarAutorEnArchivo(nombreAutor, apellidoAutor, correoAutor, institucionAutor, campoInvestigacionAutor);
            if(codUnicoAutor != null && !codUnicoAutor.isEmpty())
            {
            ...
            {
                isRunning = false;
                throw new Exception("Finalizando el programa...");
            }
            PrintConsole.o("\n");
            ...
            String codUnicoArticulo = new OperacionesGenerales().guardarArticuloEnArchivo(tituloArticulo, resumenArticulo, contenidoArticulo, palabrasClavesArticulo, codUnicoAutor);
            if(codUnicoArticulo != null && !codUnicoArticulo.isEmpty())
            {
                PrintConsole.o("Información de artículo guardada exitosamente.\n");
            }
            else
            {
                isRunning = false;
            ...
            {
                Boolean asignoRevisiores = new OperacionesGenerales().asignarRevisores(codUnicoArticulo);
                if (asignoRevisiores) {
    ...
    {   
        ...
        List<Usuario> listadoUsuario = new OperacionesGenerales().inicioSesion(userUsuario, passwordUsuario);
        ...
        {
            for (Usuario objUsuarioListado: listadoUsuario) {
            ...
            {
                boolean registraDecisionFinal = new OperacionesGenerales().registrarDecisionFinal(codUnicoArticulo);
                if(registraDecisionFinal)
                {
                    ...
                    new OperacionesGenerales().actualizarArticuloXEditor(userUsuario, passwordUsuario, rolUsuario, codUnicoPersona, codUnicoArticulo, estadoPublicacion, codUnicoPersona);
                    PrintConsole.o("Su decisión ha sido guardada exitosamente");
                }
            }
            else {
                new OperacionesGenerales().registrarDecisionRevisor(codUnicoArticulo, codUnicoPersona);
            }
        }
    }
}
